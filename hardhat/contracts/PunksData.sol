...
contract PunksData {
    ...
    function addAsset(uint8 index, bytes memory encoding, string memory name) external onlyDeployer {
    ...
    function addPunks(uint8 index, bytes memory _punks) external onlyDeployer {
        punks[index] = _punks;
    }
                ...
                for (uint256 i = 0; i < n; i++) {
                    ...
                    for (uint256 dx = 0; dx < 2; dx++) {
                        for (uint256 dy = 0; dy < 2; dy++) {
                            ...
                            if (v[2] & (1 << (dx * 2 + dy)) != 0) {
                                bytes4 c =
                                    composite(a[i * 3 + 1], pixels[p], pixels[p + 1], pixels[p + 2], pixels[p + 3]);
                                pixels[p] = c[0];
                                pixels[p + 1] = c[1];
                                pixels[p + 2] = c[2];
                                pixels[p + 3] = c[3];
